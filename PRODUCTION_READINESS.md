...
| 2026-05-26 | Claude (session 4 — prod hot-fix follow-up) | Production triage uncovered two data-quality bugs: (a) `cpp_nav_series.benchmark_value` stuck at a single value (23643.50) for 87% of 384,883 rows — NIFTY max DD reported as -0.14% / CAGR 0.01% across 4+ years; one-shot SQL backfill from `fie_v3.index_prices` (index_name='NIFTY', ~1,395 rows 2020-09→2026-05) restored per-date values, but the writer code path needed making permanent. (b) `xirr_service.compute_xirr` raised `TypeError: can't compare datetime.datetime to datetime.date` during sort for 356/363 clients because cash_flows arrived as a mixed date/datetime list; container was hot-patched. **PR opened (claude/stoic-babbage-xgPme-benchmark-xirr-fix)** to make both fixes permanent: `benchmark_service._jip_db_dsn` now layers in `sslmode=verify-full` + `sslrootcert=$RDS_CA_BUNDLE` to match `database.py`'s TLS posture, and `xirr_service.compute_xirr` normalizes every cash-flow date to `datetime.date` BEFORE the sort. Regression tests added: `tests/test_benchmark_service.py` (asserts long-range alignment produces >100 distinct values; single-value input is rejected, not propagated) and `tests/test_xirr_service.py::test_xirr_handles_mixed_date_and_datetime`. After deploy, run `POST /api/admin/recompute-risk` with `force=true` to refresh all 363 clients' risk metrics off the corrected benchmark series. |
