"""Admin router — file upload, preview, recompute risk, upload log."""

import csv
import io
import os
import tempfile
from typing import Any

from fastapi import APIRouter, Depends, HTTPException, UploadFile, status
from sqlalchemy import select, desc
from sqlalchemy.ext.asyncio import AsyncSession

from backend.database import get_db
from backend.middleware.auth_middleware import get_admin_user
from backend.models.client import Client
from backend.models.upload_log import UploadLog
from backend.schemas.admin import (
    UploadLogResponse,
    UploadPreviewResponse,
    UploadResponse,
)

router = APIRouter(prefix="/api/admin", tags=["admin-upload"])

ALLOWED_EXTENSIONS = {".xlsx", ".xls", ".csv"}
MAX_UPLOAD_SIZE = 50 * 1024 * 1024  # 50 MB


def _validate_upload(file: UploadFile) -> str:
    """Validate upload file extension. Returns the extension."""
    if file.filename is None:
        raise HTTPException(status_code=400, detail="Filename is required")
    ext = os.path.splitext(file.filename)[1].lower()
    if ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported file type '{ext}'. Allowed: {ALLOWED_EXTENSIONS}",
        )
    return ext


async def _save_and_process(
    file: UploadFile,
    admin: dict,
    db: AsyncSession,
    file_type: str,
) -> UploadResponse:
    """Common upload flow for NAV and transaction files."""
    _validate_upload(file)
    content = await file.read()
    if len(content) > MAX_UPLOAD_SIZE:
        raise HTTPException(status_code=400, detail="File exceeds 50MB limit")

    with tempfile.NamedTemporaryFile(
        delete=False, suffix=os.path.splitext(file.filename or "upload")[1]
    ) as tmp:
        tmp.write(content)
        tmp_path = tmp.name

    try:
        if file_type == "NAV":
            from backend.services.ingestion_service import ingest_nav_file
            result = await ingest_nav_file(tmp_path, db, admin["client_id"])
        else:
            from backend.services.ingestion_service import ingest_transaction_file
            result = await ingest_transaction_file(tmp_path, db, admin["client_id"])
    except ImportError:
        result = {
            "rows_processed": 0, "rows_failed": 0, "clients_affected": 0,
            "errors": [{"message": f"ingestion_service not yet implemented for {file_type}"}],
        }
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"{file_type} ingestion failed: {exc!s}",
        ) from exc
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)

    log = UploadLog(
        uploaded_by=admin["client_id"],
        file_type=file_type,
        filename=file.filename,
        rows_processed=result.get("rows_processed", 0),
        rows_failed=result.get("rows_failed", 0),
        clients_affected=result.get("clients_affected", 0),
        errors=result.get("errors", []),
    )
    db.add(log)
    await db.flush()

    return UploadResponse(
        file_type=file_type,
        filename=file.filename or "unknown",
        rows_processed=result.get("rows_processed", 0),
        rows_failed=result.get("rows_failed", 0),
        clients_affected=result.get("clients_affected", 0),
        errors=result.get("errors", []),
    )


@router.post("/upload-nav", response_model=UploadResponse)
async def upload_nav(
    file: UploadFile,
    admin: dict = Depends(get_admin_user),
    db: AsyncSession = Depends(get_db),
) -> UploadResponse:
    """Upload NAV file, parse, ingest, compute risk metrics."""
    return await _save_and_process(file, admin, db, "NAV")


@router.post("/upload-transactions", response_model=UploadResponse)
async def upload_transactions(
    file: UploadFile,
    admin: dict = Depends(get_admin_user),
    db: AsyncSession = Depends(get_db),
) -> UploadResponse:
    """Upload transaction file, parse, ingest, recompute holdings."""
    return await _save_and_process(file, admin, db, "TRANSACTIONS")


@router.post("/upload-preview", response_model=UploadPreviewResponse)
async def upload_preview(
    file: UploadFile,
    admin: dict = Depends(get_admin_user),
) -> UploadPreviewResponse:
    """Preview first 10 rows of an uploaded file with auto-column mapping."""
    ext = _validate_upload(file)
    content = await file.read()
    if len(content) > MAX_UPLOAD_SIZE:
        raise HTTPException(status_code=400, detail="File exceeds 50MB limit")

    columns: list[str] = []
    sample_rows: list[dict[str, Any]] = []
    row_count = 0

    if ext == ".csv":
        text_content = content.decode("utf-8", errors="replace")
        reader = csv.DictReader(io.StringIO(text_content))
        columns = reader.fieldnames or []
        for i, row in enumerate(reader):
            row_count += 1
            if i < 10:
                sample_rows.append(dict(row))
    else:
        import openpyxl

        with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as tmp:
            tmp.write(content)
            tmp_path = tmp.name

        try:
            wb = openpyxl.load_workbook(tmp_path, read_only=True, data_only=True)
            ws = wb.active
            if ws is not None:
                header_done = False
                for row_cells in ws.iter_rows(values_only=True):
                    vals = [str(c) if c is not None else "" for c in row_cells]
                    if not header_done:
                        columns = vals
                        header_done = True
                        continue
                    row_count += 1
                    if row_count <= 10:
                        sample_rows.append(
                            {columns[i]: vals[i] for i in range(min(len(columns), len(vals)))}
                        )
                wb.close()
        finally:
            if os.path.exists(tmp_path):
                os.unlink(tmp_path)

    # Auto-map known column names
    known = {
        "ucc": "client_code", "date": "nav_date", "corpus": "invested_amount",
        "nav": "nav_value", "liquidity %": "cash_pct",
        "high water mark": "high_water_mark",
        "script": "symbol", "exch": "exchange", "stno": "settlement_no",
    }
    auto_mapped: dict[str, str] = {}
    for col in columns:
        col_lower = col.strip().lower()
        if col_lower in known:
            auto_mapped[col] = known[col_lower]

    return UploadPreviewResponse(
        columns=columns,
        sample_rows=sample_rows,
        row_count=row_count,
        auto_mapped=auto_mapped,
    )


@router.post("/recompute-risk")
async def recompute_risk(
    client_id: int | None = None,
    admin: dict = Depends(get_admin_user),
    db: AsyncSession = Depends(get_db),
) -> dict[str, str]:
    """Trigger risk recomputation for all or a specific client."""
    try:
        from backend.services.risk_engine import run_risk_engine
    except ImportError:
        raise HTTPException(status_code=501, detail="Risk engine not yet implemented")

    if client_id is not None:
        await run_risk_engine(client_id, db)
        return {"message": f"Recomputed risk for client_id={client_id}"}

    stmt = select(Client.id).where(Client.is_active.is_(True))
    result = await db.execute(stmt)
    client_ids = [row[0] for row in result.all()]

    count = 0
    for cid in client_ids:
        try:
            await run_risk_engine(cid, db)
            count += 1
        except Exception:
            continue

    return {"message": f"Recomputed risk for {count} clients"}


@router.get("/upload-log", response_model=list[UploadLogResponse])
async def get_upload_log(
    admin: dict = Depends(get_admin_user),
    db: AsyncSession = Depends(get_db),
) -> list[UploadLogResponse]:
    """List recent upload history."""
    stmt = select(UploadLog).order_by(desc(UploadLog.uploaded_at)).limit(100)
    result = await db.execute(stmt)
    logs = list(result.scalars().all())

    return [
        UploadLogResponse(
            id=log.id,
            uploaded_by=log.uploaded_by,
            file_type=log.file_type,
            filename=log.filename,
            rows_processed=log.rows_processed,
            rows_failed=log.rows_failed,
            clients_affected=log.clients_affected,
            errors=log.errors if isinstance(log.errors, list) else [],
            uploaded_at=log.uploaded_at,
        )
        for log in logs
    ]
