...
import { useAggregateMonthlyReturns } from '@/hooks/useAggregate';
...
export default function AggregateMonthlyReturns() {
  const { data, loading, error } = useAggregateMonthlyReturns();

  if (loading) return <Skeleton />;
  if (error) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <p className="text-red-600 text-sm">Failed to load monthly returns: {error}</p>
      </div>
    );
  }
  if (!data) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <p className="text-slate-500 text-sm">No aggregate monthly return data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-3 sm:p-5 overflow-hidden">
      <h2 className="text-lg sm:text-xl font-semibold text-slate-800 mb-4">
        Aggregate Monthly Return Profile
      </h2>
      ...
      {/* Monthly bar chart (last 24 months) */}
