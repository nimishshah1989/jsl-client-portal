...
import { useAggregatePerformance } from '@/hooks/useAggregate';
...
const METRIC_LABELS = [
  {
    key: 'abs_return',
    label: 'Abs Return',
    description: 'Total percentage gain or loss over the period.',
  },
  {
    key: 'cagr',
    label: 'CAGR',
  ...
  },
  {
    key: 'max_dd',
    label: 'Max DD',
    description: 'Largest peak-to-trough decline during the period.',
  },
  {
    key: 'sharpe',
    label: 'Sharpe',
    description: 'Risk-adjusted return: (Return - Rf) / Volatility.',
  },
  {
    key: 'sortino',
    label: 'Sortino',
...
export default function AggregatePerformance({ strategy = 'COMBINED', includeInactive = false }) {
  const { data, loading, error } = useAggregatePerformance(strategy, includeInactive);

  if (loading) return <Skeleton />;
  if (error) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <p className="text-red-600 text-sm">Failed to load aggregate performance: {error}</p>
      </div>
    );
  }
  if (!data || data.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <p className="text-slate-500 text-sm">No aggregate performance data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-3 sm:p-5">
      <h2 className="text-lg sm:text-xl font-semibold text-slate-800 mb-4">
        Aggregate Performance &mdash; Portfolio vs NIFTY 50
      </h2>
